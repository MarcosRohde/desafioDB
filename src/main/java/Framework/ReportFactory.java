...
public class ReportFactory {
    ...
    public static void configReportExtent(){
        ...
        htmlReporter.config().setReportName("Relátorio de Execução dos Testes");
        htmlReporter.config().setTheme(Theme.DARK);
        htmlReporter.config().setEncoding("UTF-8");
        htmlReporter.config().setTimeStampFormat("EEEE, MMMM dd, YYYY, hh:mm a '('zzz')'");
        htmlReporter.config().setCSS(".nav-wrapper { background-color: #1E317A !important; } " +
                ".brand-logo { background-color: #1E317A !important; padding: 0 10px 0 0 !important; margin: 0 " +
                "!important; position: absolute !important } " +
                ".report-name { margin-left: 80px !important } " +
                ".blue.darken-3 { background-color: #1E317A !important; color: #FFF !important;}");
        htmlReporter.config().setJS("$('.brand-logo')" +
                ".html('<img src=\"https://i.imgur.com/qDYwkD2.png\" class=\"db-logo\"/>');");
